...
def download_whole_city(city_name: Union[str, List[str]], save_path: Path, year: str = "2023", timeout: int = 10000, tags: List[str] = TOP_LEVEL_OSM_TAGS):
    if type(city_name) == str:
        name = city_name
    else:
        name = city_name[0]
    print(name)
    area_path = save_path.joinpath(name + "/" + year)
    ...
    for tag in tqdm(tags):
        ...
        if not tag_path.exists():
            if (tag_path.parent / f"{tag}_is_empty.txt").exists():
                ...
                tag_gdf = download_whole_osm_tag(city_name, tag, timeout, year)
                if tag_gdf.empty:
                    # create a file holding whether the
                    print(
                        f"Tag: {tag} empty for city: {city_name} and year {year}")
                    with open(tag_path.parent / f"{tag}_is_empty.txt", 'w'):
                        pass
                else:
                    tag_gdf.reset_index(inplace=True)
                    tag_gdf.drop(columns=tag_gdf.columns.intersection(
                        ['nodes', 'ways']), inplace=True)
                    tag_gdf.to_feather(tag_path)
        else:
            print(f"Tag: {tag} already exists for city: {city_name} and year {year}")


def download_whole_osm_tag(
    area_name: Union[str, Dict[str, str]], tag: str, timeout: int = 10000, year: str = "2023"
) -> GeoDataFrame:
    return download_specific_tags(area_name, {tag: True}, timeout, year)


def download_specific_tags(
    area_name: Union[str, Dict[str, str]], tags: Dict[str, Union[str, bool]], timeout: int = 10000, year: str = "2023"
) -> GeoDataFrame:
    ox.settings.timeout = timeout
    ox.settings.overpass_settings = '[out:json][timeout:90][date:"' + \
        year + '-01-01T19:20:00Z"]'
